...
int MyQAuthenticationManager::request(
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (https.begin(client, url)) {
      ...
      MYQ_DETAIL_LINE("Request sent. Response %i", res);

      if (res >= 200 && res <= 299) {
        DeserializationError err;
        if (filter && filter->size() != 0) err = deserializeJson(doc, client, DeserializationOption::Filter(*filter), nestingLimit);
